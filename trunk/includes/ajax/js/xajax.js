...
function Xajax()
{
	...
	this.getRequestObject = function()
	{
		...
		if (!req && typeof ActiveXObject != "undefined")
		{
			...
			for (i = 0; i < 7; i++)
			{
				try {req = new ActiveXObject(msxmls[i]);}
				catch (e) {req = null}
			}
		}
					...
					if(formElements[i].type=='select-multiple')
					{
						for (var j = 0; j < formElements[i].length; j++)
						{
							if (formElements[i].options[j].selected == true)
								sXml += name+"="+urlencode(formElements[i].options[j].value)+"&";
						}
					}
					else
					{
						sXml += name+"="+urlencode(formElements[i].value);
					}
				}
			}
		}
	...
	this.call = function(sFunction, aArgs, sRequestType)
	{
		...
		var uri = xajaxRequestUri + '?xajaxdo=' + this.type;
		var value;
		switch(xajaxRequestType)
		{
			case xajaxDefinedGet:{
				var uriGet = "&xajax="+urlencode(sFunction);
				if (aArgs) {
					for (i = 0; i<aArgs.length; i++)
					{
						...
						if (typeof(value)=="object")
							value = this.objectToXML(value);
						uriGet += "&xajaxargs[]="+urlencode(value);
					}
				}
			...
			case xajaxDefinedPost:{
				postData = "xajax="+urlencode(sFunction);
				...
				if (aArgs) {
					for (i = 0; i <aArgs.length; i++)
					{
						...
						if (typeof(value)=="object")
							value = this.objectToXML(value);
						postData = postData+"&xajaxargs[]="+urlencode(value);
					}
				}
				} break;
			default:
