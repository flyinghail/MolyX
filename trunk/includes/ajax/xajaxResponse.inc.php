...
class xajaxResponse
{
	...
	/**
	 * Uses the script() method to add a Javascript redirect to another URL.
	 *
	...
	function call()
	{
	    ...
	    $aData = !empty($aArgs) ? $this->_buildObj($aArgs) : '';
	 ...
	 * @param string deprecated, use the createInput() method instead
	 */
	function create($sParent, $sTag, $sId, $sType="")
	{
		if ($sType)
		{
			trigger_error("The \$sType parameter of create has been deprecated.  Use the createInput() method instead.", E_USER_WARNING);
			return;
		}
