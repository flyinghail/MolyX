...
class xajax
{
	 ...
	 */
	function xajax($sRequestURI="",$sWrapperPrefix="mxajax_",$sEncoding=XAJAX_DEFAULT_CHAR_ENCODING,$bDebug=false)
	{
	...
	function processRequests()
	{
		...
		if ($this->sPreFunction) {
			if (!$this->_isFunctionCallable($this->sPreFunction)) {
				...
				$oResponse->alert('Unknown Pre-Function '. $this->sPreFunction);
			}
		}
		...
		if ($bFoundFunction)
		{
			...
			if (!$bEndRequest) {
				...
				if (is_string($sResponse)) {
					$oResponse = new xajaxResponse();
					$oResponse->alert("No XML Response Was Returned By Function $sFunctionName.\n\nOutput: ".$oResponse);
				}
